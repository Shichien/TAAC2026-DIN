...
class ModelInput(NamedTuple):
    ...
    sample_timestamps: torch.Tensor  # [B]


class SplitUserDenseProjector(nn.Module):
    """按 user_dense 的原始字段逐个投影，再融合成一个 dense token。"""

    def __init__(
        self, feature_entries: List[Tuple[int, int, int]], d_model: int
    ) -> None:
        super().__init__()
        if not feature_entries:
            raise ValueError(
                "split_user_dense_tokens requires non-empty user_dense_feature_entries"
            )

        self.feature_slices = [
            (offset, length) for _, offset, length in feature_entries
        ]
        self.projs = nn.ModuleList(
            [
                nn.Sequential(nn.Linear(length, d_model), nn.LayerNorm(d_model))
                for _, _, length in feature_entries
            ]
        )
        self.output_norm = nn.LayerNorm(d_model)
        logging.info(
            "SplitUserDenseProjector feature dims: %s",
            [length for _, length in self.feature_slices],
        )

    def forward(self, dense_feats: torch.Tensor) -> torch.Tensor:
        tokens = []
        for (offset, length), projection in zip(
            self.feature_slices, self.projs
        ):
            dense_slice = dense_feats[:, offset : offset + length]
            tokens.append(F.silu(projection(dense_slice)))
        return self.output_norm(torch.stack(tokens, dim=1).mean(dim=1))


class AlignedUserDenseResidual(nn.Module):
    """对齐的 user_int / user_dense 残差交互。"""

    def __init__(
        self,
        user_int_feature_entries: List[Tuple[int, int, int]],
        user_dense_feature_entries: List[Tuple[int, int, int]],
        user_ns_tokenizer: nn.Module,
        d_model: int,
    ) -> None:
        super().__init__()
        self.d_model = d_model
        self.user_int_feature_entries = user_int_feature_entries
        self.user_ns_tokenizer = user_ns_tokenizer

        user_int_fid_to_idx = {
            fid: idx for idx, (fid, _, _) in enumerate(user_int_feature_entries)
        }
        user_dense_fid_to_entry = {
            fid: (offset, length)
            for fid, offset, length in user_dense_feature_entries
        }
        overlap_fids = sorted(
            set(user_int_fid_to_idx).intersection(user_dense_fid_to_entry)
        )

        self.metas: List[Tuple[int, int, int, int]] = []
        self.dense_projs = nn.ModuleList()
        self.pair_projs = nn.ModuleList()
        for fid in overlap_fids:
            feature_idx = user_int_fid_to_idx[fid]
            emb_real_idx = self.user_ns_tokenizer._emb_index[feature_idx]
            if emb_real_idx == -1:
                continue
            dense_offset, dense_length = user_dense_fid_to_entry[fid]
            self.metas.append((fid, feature_idx, dense_offset, dense_length))
            self.dense_projs.append(
                nn.Sequential(
                    nn.Linear(dense_length, d_model), nn.LayerNorm(d_model)
                )
            )
        ...
        if self.metas:
            logging.info(
                "Aligned user pair features enabled for fids: %s",
                [fid for fid, _, _, _ in self.metas],
            )
        else:
            logging.warning(
                "Aligned user pair features enabled, but no usable overlapping "
                "user_int/user_dense fids were found."
            )

    def _embed_sparse_feature(
        self, int_feats: torch.Tensor, feature_idx: int
    ) -> torch.Tensor:
        emb_real_idx = self.user_ns_tokenizer._emb_index[feature_idx]
        emb_layer = self.user_ns_tokenizer.embs[emb_real_idx]
        offset = self.user_int_feature_entries[feature_idx][1]
        length = self.user_int_feature_entries[feature_idx][2]
        if length == 1:
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        if not self.metas:
            return inputs.user_dense_feats.new_zeros(
                inputs.user_dense_feats.shape[0], self.d_model
            )

        pair_tokens = []
        for meta, dense_proj, pair_proj in zip(
            self.metas, self.dense_projs, self.pair_projs
        ):
            _, feature_idx, dense_offset, dense_length = meta
            sparse_emb = self._embed_sparse_feature(
                inputs.user_int_feats, feature_idx
            )
            dense_slice = inputs.user_dense_feats[
                :, dense_offset : dense_offset + dense_length
            ]
            dense_emb = F.silu(dense_proj(dense_slice))
            pair_raw = torch.cat(
                [sparse_emb * dense_emb, torch.abs(sparse_emb - dense_emb)],
                dim=-1,
            )
            pair_tokens.append(F.silu(pair_proj(pair_raw)))
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        use_absolute_time_features: bool = True,
        timestamp_utc_offset_hours: int = 8,
        user_int_feature_entries: Optional[List[Tuple[int, int, int]]] = None,
        user_dense_feature_entries: Optional[List[Tuple[int, int, int]]] = None,
        split_user_dense_tokens: bool = False,
        use_item_context: bool = False,
        use_aligned_user_pair_features: bool = False,
    ) -> None:
        super().__init__()
        ...
        self.use_absolute_time_features = use_absolute_time_features
        self.timestamp_utc_offset_hours = int(timestamp_utc_offset_hours)
        self.split_user_dense_tokens = split_user_dense_tokens
        self.use_item_context = use_item_context
        self.use_aligned_user_pair_features = use_aligned_user_pair_features
        ...
        if self.has_user_dense:
            if split_user_dense_tokens:
                if not user_dense_feature_entries:
                    raise ValueError(
                        "split_user_dense_tokens=True requires user_dense_feature_entries"
                    )
                self.user_dense_proj = SplitUserDenseProjector(
                    feature_entries=user_dense_feature_entries, d_model=d_model
                )
            else:
        ...
        self.aligned_user_pair_residual: Optional[AlignedUserDenseResidual] = (
            None
        )
        if self.use_aligned_user_pair_features:
            if not user_int_feature_entries or not user_dense_feature_entries:
                raise ValueError(
                    "use_aligned_user_pair_features=True requires "
                    "user_int_feature_entries and user_dense_feature_entries"
                )
            self.aligned_user_pair_residual = AlignedUserDenseResidual(
                user_int_feature_entries=user_int_feature_entries,
                user_dense_feature_entries=user_dense_feature_entries,
                user_ns_tokenizer=self.user_ns_tokenizer,
                d_model=d_model,
            )
        if self.use_item_context:
            self.item_context_proj = nn.Sequential(
        ...
        if self.use_absolute_time_features:
            self.abs_hour_embedding = nn.Embedding(24, d_model)
            self.abs_weekday_embedding = nn.Embedding(7, d_model)
            self.abs_weekend_embedding = nn.Embedding(2, d_model)
            self.abs_night_embedding = nn.Embedding(2, d_model)
            self.abs_time_context_norm = nn.LayerNorm(d_model)
    ...
    def _init_params(self) -> None:
        ...
        if self.use_absolute_time_features:
            for emb in (
                self.abs_hour_embedding,
                self.abs_weekday_embedding,
                self.abs_weekend_embedding,
                self.abs_night_embedding,
            ):
                nn.init.xavier_normal_(emb.weight.data)
    ...
    def _build_absolute_time_context(
        ...
        context = (
            ...
            + self.abs_weekend_embedding(is_weekend)
            + self.abs_night_embedding(is_night)
        )
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        if self.use_absolute_time_features:
            abs_time_context = self._build_absolute_time_context(
                inputs.sample_timestamps
            )
            ns_tokens = ns_tokens + abs_time_context.unsqueeze(1)
