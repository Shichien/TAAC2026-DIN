...
_FALLBACK_MODEL_CFG = {
    ...
    "user_ns_tokens": 0,
    "item_ns_tokens": 0,
    "use_absolute_time_features": True,
    "timestamp_utc_offset_hours": 8,
    "split_user_dense_tokens": False,
    "use_item_context": False,
    "use_aligned_user_pair_features": False,
}
...
def build_model(
    ...
    model = PCVRHyFormer(
        ...
        user_int_feature_entries=dataset.user_int_schema.entries,
        user_dense_feature_entries=dataset.user_dense_schema.entries,
...
def _extract_global_step(path: str) -> int:
    """从 ``.../global_step1234...`` 路径中提取 step；失败时返回 -1。"""
...
def resolve_ckpt_dir_and_path(
    model_output_path: Optional[str],
) -> Tuple[Optional[str], Optional[str]]:
    """解析真正的 ckpt 目录和 ``model.pt`` 路径。

    兼容两类平台传参：
    1. ``MODEL_OUTPUT_PATH`` 直接指向包含 ``model.pt`` 的叶子目录；
    2. ``MODEL_OUTPUT_PATH`` 指向 ckpt 根目录，此时需要递归寻找
       ``global_step*.best_model/model.pt`` 或其他 ``model.pt``。
    """
    if not model_output_path or not os.path.isdir(model_output_path):
        return None, None

    direct_pt = os.path.join(model_output_path, "model.pt")
    if os.path.exists(direct_pt):
        return model_output_path, direct_pt

    direct_pts = sorted(glob.glob(os.path.join(model_output_path, "*.pt")))
    if direct_pts:
        return model_output_path, direct_pts[0]

    best_candidates = sorted(
        glob.glob(
            os.path.join(model_output_path, "**", "*.best_model", "model.pt"),
            recursive=True,
        ),
        key=_extract_global_step,
    )
    if best_candidates:
        ckpt_path = best_candidates[-1]
        return os.path.dirname(ckpt_path), ckpt_path

    any_candidates = sorted(
        glob.glob(
            os.path.join(model_output_path, "**", "model.pt"), recursive=True
        ),
        key=_extract_global_step,
    )
    if any_candidates:
        ckpt_path = any_candidates[-1]
        return os.path.dirname(ckpt_path), ckpt_path
...
def main() -> None:
    ...
    model_dir, ckpt_path = resolve_ckpt_dir_and_path(model_root)
    if model_dir is None or ckpt_path is None:
        raise FileNotFoundError(
            f"Could not resolve checkpoint from MODEL_OUTPUT_PATH={model_root!r}. "
            "Expected either a directory containing model.pt directly, or a ckpt root "
            "containing global_step*.best_model/model.pt."
        )
