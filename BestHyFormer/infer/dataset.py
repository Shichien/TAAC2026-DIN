        ...
        for f in pq_files:
            ...
            for batch in pf.iter_batches(columns=["timestamp"]):
                timestamp_values.append(
                    batch.column(0).to_numpy().astype(np.int64)
                )
        if not timestamp_values:
            raise ValueError("No timestamp values found for timestamp split")
        timestamps = np.concatenate(timestamp_values)
        split_index = max(
            1,
            min(
                len(timestamps) - 1, int(len(timestamps) * (1.0 - valid_ratio))
            ),
        )
        split_timestamp = int(
            np.partition(timestamps, split_index)[split_index]
        )
        train_timestamp_range = (None, split_timestamp)
        valid_timestamp_range = (split_timestamp, None)
        train_rows = int((timestamps < split_timestamp).sum())
        valid_rows = int((timestamps >= split_timestamp).sum())
        train_row_group_range = (0, total_rgs)
        valid_row_group_range = (0, total_rgs)
        logging.info(
            f"Timestamp split: train timestamp < {split_timestamp} ({train_rows} rows), "
            f"valid timestamp >= {split_timestamp} ({valid_rows} rows), valid_ratio={valid_ratio}"
        )
    else:
